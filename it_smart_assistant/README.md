...
Each generated project includes a CLI tool named `it_smart_assistant`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run it_smart_assistant server run --reload     # Start dev server
uv run it_smart_assistant server routes           # Show all routes
# Database commands
uv run it_smart_assistant db migrate -m "message" # Create migration
uv run it_smart_assistant db upgrade              # Apply migrations
uv run it_smart_assistant db downgrade            # Rollback migration
# User commands
uv run it_smart_assistant user create-admin       # Create admin user
uv run it_smart_assistant user create             # Create regular user
uv run it_smart_assistant user list               # List all users
```
...
# Server
uv run it_smart_assistant server run --reload
uv run it_smart_assistant server routes

# Database (Alembic wrapper)
uv run it_smart_assistant db init
uv run it_smart_assistant db migrate -m "Add users"
uv run it_smart_assistant db upgrade

# Users
uv run it_smart_assistant user create-admin       # Create admin (interactive)
uv run it_smart_assistant user create             # Create user (interactive)
uv run it_smart_assistant user list               # List all users
```
...
```bash
uv run it_smart_assistant cmd seed --count 100
uv run it_smart_assistant cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend it_smart_assistant server run --reload` |
| Start prod server | `uv run --directory backend it_smart_assistant server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend it_smart_assistant server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend it_smart_assistant db migrate -m "message"` |
| Apply migrations | `uv run --directory backend it_smart_assistant db upgrade` |
| Rollback migration | `uv run --directory backend it_smart_assistant db downgrade` |
| Show current | `uv run --directory backend it_smart_assistant db current` |
| Show history | `uv run --directory backend it_smart_assistant db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend it_smart_assistant user create-admin` |
| Create user | `uv run --directory backend it_smart_assistant user create` |
| List users | `uv run --directory backend it_smart_assistant user list` |
